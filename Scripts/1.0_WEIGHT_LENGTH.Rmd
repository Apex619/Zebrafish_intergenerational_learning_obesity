...
emmeans_weight <- emmeans(Model_Body_Weight, ~c(Set, Sex))
...
Model_Weight_1 <- lme(Weight_g ~ Set + Sex + Age_Weeks, random = ~ 1| Fish_ID, data = Weight_Length_Data) #Mixed model without variance structure
summary(Model_Weight_1)
Model_Weight_2 <- lme(Weight_g ~ Set + Sex + Age_Weeks, random = ~ 1| Fish_ID, weights = varIdent(form=~1|Set), data = Weight_Length_Data) #mixed model with variance structure varIdent
