...
Endpoint = c("Total distance travelled", "Total distance travelled", "Total distance travelled", "Total distance travelled"))
...
Endpoint = c("Time spent in low zone", "Time spent in low zone", "Time spent in low zone", "Time spent in low zone"))
...
# 
# Overall_Anxiety_Plotting_Data_F1 
...
```{r}
Overall_Data_Joined1_reorder <- Overall_Data_Joined1 %>%
  mutate(Group = fct_relevel(Group, 
            "F1T", "F1P", "F1M", 
            "F1C"))


Overall_plot_F1 <- ggplot(Overall_Data_Joined1_reorder, aes(x=Endpoint, y=r, colour=Group)) +
...
ylim(-0.25, 0.9)+
  ...
  annotate("text", x = 1.02, y = 0.70, label = "*", size = 7)+
  annotate("text", x = 0.94, y = 0.49, label = "*", size = 7)+#low dur
  annotate("text", x = 2.10, y = 0.73, label = "*", size = 7)+ #tot_dist
  annotate("text", x = 1.94, y = 0.51, label = "*", size = 7)+
...
#indiv means
individual_means_low <- Anxiety_Joined_reordered %>%
    group_by(Fish_ID, Sex, Tank) %>%
    dplyr::summarize(Mean = mean(low_dur, na.rm=TRUE))

individual_means_tot <- Anxiety_Joined_reordered %>%
    group_by(Fish_ID, Sex, Tank) %>%
    dplyr::summarize(Mean = mean(tot_dist, na.rm=TRUE))


Anxiety_Joined_reordered <- Anxiety_Joined %>%
  mutate(Tank = fct_relevel(Tank, 
            "Within Control", "Maternal", "Paternal", 
            "Within Treatment"))

low_dur_violin_indiv <- violin_plot_custom_hamza2(individual_means_low,individual_means_low$Mean,"B) Time spent in low zone", "Seconds")+theme(axis.text.x=element_blank(),axis.ticks = element_blank())+
    stat_summary(fun=mean, geom="point", shape=18, size=3, color="red", fill="red")

tot_dist_violin_indiv <- violin_plot_custom_hamza2(individual_means_tot, individual_means_tot$Mean, "A) Total distance travelled", "Distance (cm)")+theme(axis.text.x=element_blank(),axis.ticks = element_blank())+
    stat_summary(fun=mean, geom="point", shape=18, size=3, color="red", fill="red")


low_dur_violin <- violin_plot_custom_hamza2(Anxiety_Joined_reordered,Anxiety_Joined_reordered$low_dur,"B) Time spent in low zone", "Seconds")+theme(axis.text.x=element_blank(),axis.ticks = element_blank())+
    stat_summary(fun=mean, geom="point", shape=18, size=3, color="red", fill="red")

tot_dist_violin <- violin_plot_custom_hamza2(Anxiety_Joined_reordered, Anxiety_Joined_reordered$tot_dist, "A) Total distance travelled", "Distance (cm)")+theme(axis.text.x=element_blank(),axis.ticks = element_blank())+
    stat_summary(fun=mean, geom="point", shape=18, size=3, color="red", fill="red")
...
Joined_violins_new_design <- tot_dist_violin_indiv + low_dur_violin_indiv + plot_layout(ncol =1, nrow=2)+ theme(legend.position = c(1.5,0.4))+ theme(
