...
Overall_plot_F1 <- ggplot(Overall_Data_Joined1, aes(x=Endpoint, y=r, colour=Group)) +
...
ylim(-0.25, 0.9)+
  ...
  scale_colour_manual(values=c("#009E73","#E69F00", "#56B4E9","#999999"))+
  ...
  annotate("text", x = 0.94, y = 0.70, label = "*", size = 7)+
  annotate("text", x = 1.02, y = 0.49, label = "*", size = 7)+#low dur
  annotate("text", x = 1.87, y = 0.73, label = "*", size = 7)+ #tot_dist
  annotate("text", x = 2.02, y = 0.51, label = "*", size = 7)+
...
low_dur_violin <- violin_plot_custom_hamza2(Anxiety_Joined,Anxiety_Joined$low_dur,"B) Time spent in low zone", "Seconds")+theme(axis.text.x=element_blank(),axis.ticks = element_blank())+
    stat_summary(fun=mean, geom="point", shape=18, size=3, color="red", fill="red")

tot_dist_violin <- violin_plot_custom_hamza2(Anxiety_Joined, Anxiety_Joined$tot_dist, "A) Total distance travelled", "Distance (cm)")+theme(axis.text.x=element_blank(),axis.ticks = element_blank())+
    stat_summary(fun=mean, geom="point", shape=18, size=3, color="red", fill="red")
...
Joined_violins_new_design <- tot_dist_violin + low_dur_violin + plot_layout(ncol =1, nrow=2)+ theme(legend.position = c(1.5,0.4))+ theme(
